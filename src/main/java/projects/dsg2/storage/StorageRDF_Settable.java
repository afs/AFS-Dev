...
/** A {@link StorageRDF_Wrapper}-like facility that allows the target to be swapped */
public class StorageRDF_Settable implements StorageRDF {

    private AtomicReference<StorageRDF> ref = new AtomicReference<>(null);

    public StorageRDF_Settable(StorageRDF other) {
        ref.set(other);
    }
    
    public StorageRDF get() { return ref.get(); }

    public StorageRDF swap(StorageRDF newOther) {
        return ref.getAndSet(newOther);
    }
